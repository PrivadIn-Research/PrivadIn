...
function isBetweenMinutes(current, start, end) {
    if (start <= end)
...
function assertActiveWorkTime(schedule, now) {
...
function encryptSalaryCents(value) {
...
function decryptSalaryCents(data) {
    if (!data?.salary?.ciphertext || !data.salary.iv || !data.salary.tag)
        return 0;
    const decipher = createDecipheriv("aes-256-gcm", encryptionKey(), Buffer.from(data.salary.iv, "base64"));
...
function dailyWorkMinutes(schedule) {
    const workStart = minutesOfDay(schedule.horarioInicioExpediente);
    const workEnd = minutesOfDay(schedule.horarioFimExpediente);
    const lunchStart = minutesOfDay(schedule.horarioInicioAlmoco);
    const lunchEnd = minutesOfDay(schedule.horarioFimAlmoco);
...
export const registerPoopSecure = onCall(async (request) => {
    ...
    await db.runTransaction(async (transaction) => {
        ...
        const cooldownUntil = user.cooldownUntil;
